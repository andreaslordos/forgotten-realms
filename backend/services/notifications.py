...
async def broadcast_arrival(player):
    ...
    for sid, session_data in SESSIONS.items():
        ...
        if other_player.current_room == room_id and other_player != player:
            await send_msg(sid, f"{display_name} the {display_level} has just arrived.")
...
async def broadcast_departure(room_id, departing_player):
    ...
    for sid, session_data in SESSIONS.items():
        if ('player' in session_data and 
            ...
            await send_msg(sid, f"{departing_player.name} the {departing_player.level} has left")
...
async def broadcast_logout(player):
    ...
    global SESSIONS, send_msg
    if not SESSIONS or not send_msg:
        logger.warning("Attempted to broadcast logout but context not initialized")
        return
    ...
    for sid, session_data in SESSIONS.items():
        ...
        if not other_player:
            continue  # Skip sessions that haven't authenticated.
        if other_player.current_room == room_id and other_player != player:
            await send_msg(sid, f"{display_name} the {player.level} has just passed on.")