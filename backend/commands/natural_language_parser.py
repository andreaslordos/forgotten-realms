...
def tokenize(command_str: str) -> List[Token]:
    ...
    if command_str.startswith('"'):
        logger.debug(
            f"Detected quote-prefixed command, mapping to say: '{command_str}'"
        )
...
class VocabularyManager:
    ...
    def _initialize_vocabularies(self) -> None:
        ...
        self.abbreviations["ne"] = {
            "default": "northeast",
            "in_prep_position": "northeast",
        }
    ...
    def add_abbreviation(
        ...
        logger.debug(
            f"Added abbreviation: {abbreviation} -> {full_word}"
        ...
        if word in self.abbreviations:
            ...
            if isinstance(abbrev_value, dict):
                ...
                if position == 0:
                    ...
                    logger.debug(
                        f"Context-based expansion: pos={position}, total={total_words}, "
                ...
                if expanded:
                    word = expanded
                    logger.debug(
                        f"Expanded context-aware abbreviation: {original} -> {word} (position {position})"
                    )
            else:
                # Simple abbreviation
                word = abbrev_value
    ...
    def is_standard_preposition(self, word: str) -> bool:
        ...
        return (
            word.lower() in self.prepositions
            and self.preposition_types.get(word.lower()) == "standard"
        )
    ...
    def is_reversed_preposition(self, word: str) -> bool:
        ...
        return (
            word.lower() in self.prepositions
            and self.preposition_types.get(word.lower()) == "reversed"
        )
...
class SyntaxPattern:
    ...
    def _parse_pattern(self) -> List[Dict[str, str]]:
        ...
        logger.debug(
            f"Parsed pattern '{self.pattern_str}' into components: {components}"
        )
        return components
    ...
    def matches(self, tokens: List[Token]) -> Tuple[bool, Dict[str, Any]]:
        ...
        logger.debug(
            f"Trying to match {len(tokens)} tokens against pattern '{self.pattern_str}'"
        )
        ...
        while pattern_index < len(self.components) and token_index < len(tokens):
            ...
            if "value" in component:
                ...
                if (
                    ...
                    and tokens[token_index].value.lower() == component["value"].lower()
                ):
                    logger.debug(
                        f"Matched literal '{component['value']}' at position {token_index}"
                    )
                    token_index += 1
                    pattern_index += 1
                else:
                    ...
                    logger.debug(
                        f"Failed to match literal '{component['value']}' against token '{tokens[token_index].value}'"
                    )
                    return False, {}
                ...
                if component_type == "VERB":
                    ...
                    if pattern_index + 1 >= len(self.components):
                        ...
                        while token_index < len(tokens) and (
                            "value" not in self.components[pattern_index + 1]
                            or tokens[token_index].value
                            != self.components[pattern_index + 1]["value"]
                        ):
                            end_idx += 1
                            token_index += 1
                    ...
                    if start_idx < end_idx:
                        ...
                        components_dict["subject"] = " ".join(
                            t.value for t in subject_tokens
                        )
                    ...
                    if start_idx < end_idx:
                        ...
                        components_dict["instrument"] = " ".join(
                            t.value for t in instrument_tokens
                        )
                        logger.debug(
        ...
        if pattern_index >= len(self.components):
            ...
            for i, component in enumerate(self.components):
                if (
                    "value" in component
                    and component["value"] in vocabulary_manager.prepositions
                ):
            ...
            logger.debug(
                f"Successfully matched pattern '{self.pattern_str}': {components_dict}"
            )
...
ONE_WORD_COMMAND = CommandStructure([SyntaxPattern("VERB", priority=10)])
...
REVERSED_SYNTAX_COMMAND = CommandStructure(
    [
        SyntaxPattern(
            "VERB INSTRUMENT to SUBJECT", priority=15
        ),  # Also higher priority
...
MOVEMENT_COMMAND = CommandStructure([SyntaxPattern("VERB", priority=10)])
...
class ObjectBinder:
    ...
    def bind_subject(
        ...
        player_obj = self.bind_entity(
            subject_str, "subject", player, game_state, context
        )
        if player_obj:
            return player_obj
        ...
        for other_player in get_players_in_room(player.current_room, game_state):
            if subject_str.lower() == other_player.name.lower():
                logger.debug(
                    f"Found exact matching player in room: {other_player.name}"
                )
                return other_player
    ...
    def bind_instrument(
        ...
        # First try to bind to a player using our helper function
        player_obj = self.bind_entity(
            instrument_str, "instrument", player, game_state, context
        )
        if player_obj:
            return player_obj
...
class NaturalLanguageParser:
    ...
    def parse(
        ...
        if tokens and player and game_state:
            ...
            for other_player in players_in_room:
                if other_player.name.lower() == first_token:
                    logger.debug(
                        f"Detected potential direct message to player: {other_player.name}"
                    )
        ...
        if tokens:
            ...
            is_communication_cmd = expanded_verb in [
                "say",
                "tell",
                "shout",
                "act",
            ...
            if not (is_communication_cmd or is_direct_message):
                ...
                for i in range(1, len(tokens)):
                    ...
                    expanded_value = self.vocabulary.expand_word(
                        original_value, i, total_words
                    )
                    if original_value != expanded_value:
                        tokens[i].value = expanded_value
                        logger.debug(
                            f"Expanded token at position {i}: '{original_value}' -> '{expanded_value}'"
                        )

            # Handle special 'go' cases
            if original_verb.lower() == "go" and len(tokens) > 1:
                direction = tokens[1].value.lower()
                expanded_direction = self.vocabulary.expand_word(
                    direction, 1, total_words
                )

                logger.debug(
                    f"Checking for direction after 'go': '{direction}' -> '{expanded_direction}'"
                )
        ...
        for pattern in all_patterns:
            logger.debug(
                f"Trying pattern: '{pattern.pattern_str}' with priority {pattern.priority}"
            )
            matched, bindings = pattern.matches(tokens)
            if matched:
                parsed_cmd = bindings
                logger.debug(
                    f"Matched pattern '{pattern.pattern_str}' with priority {pattern.priority}: {bindings}"
                )
                break
        ...
        if not parsed_cmd and tokens:
            ...
            if len(tokens) == 2:
                logger.debug(
                    "No pattern matched, but have 2 tokens. Using VERB SUBJECT fallback."
                )
                parsed_cmd = {"verb": tokens[0].value, "subject": tokens[1].value}
            # Otherwise just use the verb
            else:
                logger.debug(
                    f"No command structure matched. Using verb-only default: '{tokens[0].value}'"
                )
            ...
            if verb in ["say", "tell", "shout", "act", "whisper"]:
                ...
                if verb == "tell" and "subject" in parsed_cmd:
                    ...
                    if len(parts) > 2:
                        ...
                        logger.debug(
                            f"Using original message for tell command: {parsed_cmd['instrument']}"
                        )
                # For other communication commands
                    ...
                    if len(parts) > 1:
                        ...
                        logger.debug(
                            f"Using original message for {verb} command: {parsed_cmd['subject']}"
                        )
        ...
        if not (
            is_direct_message
            or parsed_cmd.get("verb") in ["say", "tell", "shout", "act", "whisper"]
        ):
            self.context.update(
