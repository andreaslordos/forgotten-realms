...
async def handle_password(cmd, player, game_state, player_manager, online_sessions, sio, utils):
    ...
    if pwd_change['stage'] == 'old_password':
        ...
        if not command_text.strip():
            await utils.send_message(sio, current_sid, "Password cannot be blank. What is your present password?")
            return ""
            
        # Validate the old password
        try:
            auth_manager = player_manager.auth_manager if hasattr(player_manager, 'auth_manager') else None
        ...
        if not confirm_password:
            await utils.send_message(sio, current_sid, "Password confirmation cannot be blank. Please confirm your password:")
            return ""
        ...
        if confirm_password == pwd_change['new_password']:
            ...
            try:
                ...
                auth_manager.credentials[username] = auth_manager.hash_password(username, pwd_change['new_password'])
...
async def handle_password_input(cmd, player, game_state, player_manager, online_sessions, sio, utils):
    ...
    if current_sid and 'pwd_change' in online_sessions[current_sid]:
        ...
        return await handle_password(new_cmd, player, game_state, player_manager, online_sessions, sio, utils)
