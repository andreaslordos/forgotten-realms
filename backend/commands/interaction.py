    ...
    try:
        ...
        if not primary_item:
            if reversed_syntax:
                ...
                if not primary_item:
                    for item in current_room.get_items(game_state):
                        if subject and subject.lower() in item.name.lower() and hasattr(item, 'interactions'):
                            primary_item = item
            ...
            if primary_item and 'secondary_item_name' in locals() and secondary_item_name:
                for item in player.inventory:
